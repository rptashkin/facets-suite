...
#msk_impact_468 <- scan('/dmp/data/mskdata/interval-lists/production/genelist', what="", quiet = TRUE)
...
#IMPACT468_targets <- suppressWarnings(fread('grep -v ^@ /dmp/data/mskdata/interval-lists/production/picard_targets.interval_list'))
...
get_gene_level_calls <- function(cncf_files,
                                 ...
                                 fun.rename = function(filename){unlist(strsplit(basename(filename), "\\."))[1]}){
...
#  if(method == 'cncf'){
  ...
  gene_level <- fo_impact[!Hugo_Symbol %in% c("Tiling", "FP", "intron"),
                          list(chr = unique(chr),
                               ...
                               Nprobes = .N),
                          keyby=list(Tumor_Sample_Barcode, Hugo_Symbol, tcn=tcn, lcn=lcn, cf=cf.em,
...
if(!interactive()){
  ...
  #parser$add_argument('-g', '--genelist', type='character', help='IMPACT341/410/468 geneList.')
  parser$add_argument('-t', '--targetFile', type='character', help='Picard interval list file of gene target coordinates')
  ...
  geneTargets = geneTargets[V5 %like% 'target']
